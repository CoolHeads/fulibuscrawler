...
def GetUrl(number):
    ...
    i = 1
    try:
        response = requests.get('https://fuliba2021.net/{}.html'.format(number))
        soup = BeautifulSoup(response.content, 'lxml')
        page = len(soup.find('div', attrs={'class': 'article-paging'}).find_all('a')) + 2
        for i in range(1,page):
            if i != 1:
                response = requests.get('https://fuliba2021.net/{}.html/{}'.format(number,i))
                soup = BeautifulSoup(response.content, 'lxml')
        ...
        print(e)
        print('{0}期网站无法加载，链接：https://fuliba2021.net/{0}.html/{1}'.format(number, i))
    return dict
